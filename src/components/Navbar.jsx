...
const Navbar = ({bgColor,border}) => {
    ...
    return (
      <nav className={`navbar px-4 fixed h-[50px] w-screen top-0 left-0  flex  items-center py-1  z-20 bg-${bgColor} ${border ? "border" : ""} `}>
         ...
         <div className='w-screen flex justify-between items-center max-w-8xl '>
            ...
            >
              <img src="https://api.dicebear.com/5.x/initials/svg?seed=querfy%" alt="logo" className='w-8 h-8 object-contain rounded-md' /> 
              <p className='text-black text-[18px] font-bold cursor-pointer '>Querify</p>
            </div>
            ...
            >
              {navLinks.map((link)=> (
                <li 
                  key={link.id} 
                  className={`flex items-center gap-4 text-[14px] font-normal tracking-wider text-black cursor-pointer`}
